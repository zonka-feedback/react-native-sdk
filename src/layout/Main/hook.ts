...
export const useInitializeZFSdk = () => {
  const { setBaseUrl, openModal, setOpenModal } = useContext(SDKContext);
  const { getZonkaFeedbackSurveyUrlString } = useZfSurveyUrlCreate();

  const init = async (token: string, region: string) => {
    if (!!token && !!region) {
      const baseUrl = generateSurveyUrl(token, region, setBaseUrl);
      storeData(Constant.ZF_REGION, region);
      saveFirstSeen();
      saveCookieId();
      getZonkaFeedbackSurveyUrlString(token, baseUrl as string);
