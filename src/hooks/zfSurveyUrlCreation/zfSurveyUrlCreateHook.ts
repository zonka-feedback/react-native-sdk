...
export const useZfSurveyUrlCreate = () => {
  const { setUrl } = useContext(SDKContext);
  const [, setCustomVariableString] = useState("");

  const getZonkaFeedbackSurveyUrlString = async (
    token: string,
    baseUrl: string,
    deviceDetails = false,
    getCustomAttributes = {}
    ...
    if (baseUrl) {
      ...
      if (deviceDetails) {
        setCustomVariableString("");
        const variableString = addCustomParam(
          await AppUtils.getHiddenVariables(),
          token
        );
        setCustomVariableString(variableString);
        url = url + variableString;
  ...
  const addCustomParam = (hashMap: any, token: string) => {
    let variableString = "";
  ...
  return { getZonkaFeedbackSurveyUrlString };
};
