...
const AppUtils = {
  ...
  getLocalIpAddress: async function () {
    try {
      const connectionInfo = await DeviceInfo.getIpAddress();
  ...
  getHiddenVariables: async function () {
    const hashMap = {
      ...
      [Constant.TIME_ZONE]: this.getTimeZone(),
      ...
      [Constant.DEVICE_OS_VERSION]: await DeviceInfo.getBaseOs(),
      [Constant.SCREEN_NAME]: this.getScreenName()
    };
    return hashMap;
  },
